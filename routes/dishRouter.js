...
.post(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    ...
    .then((dish) => {
        if (dish != null) {
            req.body.author = req.user._id; //extract user info from authentication to populate the user document
...
.put(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    ...
    .then((dish) => {
        if (dish != null && dish.comments.id(req.params.commentId) != null) {
            ...
            dish.save()
            .then((dish) => {
                //doing one more search to populate the author field with user document
                Dishes.findById(dish._id) 
                .populate('comments.author')
                .then((dish) => {
                    res.statusCode = 200;
